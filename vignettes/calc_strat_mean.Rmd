...
```{r functions, echo = FALSE, fig.cap = "Subfunctions and data flow for the `calc_stratified_mean` function."}
par(mar = rep(1, 4))
openplotmat(main = '')
pos <- coordinates(c(4, 4, 4, 4))
...
straightarrow(from = pos[4, ], to = pos[8, ],               arr.pos = apos)

#Polygon/area flow
straightarrow(from = pos[9, ], to = pos[10, ]  - c(0.06, 0), arr.pos = apos)
straightarrow(from = pos[10, ], to = pos[11, ] - c(0.06, 0), arr.pos = apos)
straightarrow(from = pos[11, ], to = pos[2, ]  - c(0, 0.06), arr.pos = apos)

#Optional post stratify
straightarrow(from = pos[1, ], to = pos[5, ],              arr.pos = apos, lty = 3)
straightarrow(from = pos[9, ], to = pos[5, ] - c(0, 0.06), arr.pos = apos, lty = 3)
straightarrow(from = pos[5, ], to = pos[6, ] - c(0.06, 0), arr.pos = apos, lty = 3)
straightarrow(from = pos[6, ], to = pos[2, ] - c(0, 0.06), arr.pos = apos, lty = 3)

#Boxes
#Functions
textrect(mid = pos[2, ],  radx = edge, rady = edge, lab = 'strat_prep',   
         cex = text_size, shadow.size = 0, box.col = fun.col)
textrect(mid = pos[4, ],  radx = edge, rady = edge, lab = 'strat_mean',   
         cex = text_size, shadow.size = 0, box.col = fun.col)
textrect(mid = pos[5, ],  radx = edge, rady = edge, lab = 'post_strat',   
         cex = text_size, shadow.size = 0, box.col = fun.col)
textrect(mid = pos[10, ], radx = edge, rady = edge, lab = 'get_area',   
         cex = text_size, shadow.size = 0, box.col = fun.col)

#Data objects
textround(mid = pos[1, ], radx = edge - .08, rady = edge, lab = 'survdat', 
          cex = text_size, shadow.size = 0, box.col = data.col)
textround(mid = pos[9, ], radx = edge - .08, rady = edge, lab = 'areaPolygon',
          cex = text_size, shadow.size = 0, box.col = data.col)

#R Objects
texthexa(mid = pos[3, ],  radx = edge, rady = edge, lab = 'prepData',
          cex = text_size, shadow.size = 0, box.col = obj.col)
texthexa(mid = pos[6, ],  radx = edge, rady = edge, lab = 'poststratData',
          cex = text_size, shadow.size = 0, box.col = obj.col)
texthexa(mid = pos[8, ],  radx = edge, rady = edge, lab = 'stratmeanData',
          cex = text_size, shadow.size = 0, box.col = obj.col)
texthexa(mid = pos[11, ], radx = edge, rady = edge, lab = 'strataArea',
          cex = text_size, shadow.size = 0, box.col = obj.col)

#Legend
straightarrow(from = pos[13, ] + c(0, 0.04) - c(0.08, 0), to = pos[13, ] + c(0, 0.04), lty = 1, arr.pos = 1)
straightarrow(from = pos[13, ] - c(0.08, 0.04), to = pos[13, ] - c(0, 0.04), lty = 3, arr.pos = 1)
textempty(mid = pos[13, ] + c(0, 0.04), lab = 'primary \npathway', cex = text_size)
textempty(mid = pos[13, ] - c(0, 0.04) + c(0, 0), lab = 'optional \npathway', cex = text_size)
textround(mid = pos[14, ], radx = 0.02, rady = 0.04, lab = 'data',
         cex = text_size, shadow.size = 0, box.col = data.col)
textrect(mid = pos[15, ], radx = 0.04, rady = 0.04, lab = 'function',
         cex = text_size, shadow.size = 0, box.col = fun.col)
texthexa(mid = pos[16, ], radx = 0.05, rady = 0.04, lab = 'R object',
         cex = text_size, shadow.size = 0, box.col = obj.col)
```
