...
* Following functions, previously internal, are now external functions available to the user 
  * `post_strat`
  * `strat_mean`
  * `strat_prep`
  * `swept_area`
...
* Printing of intermediate function output has been removed from `calc_stratified_mean()`

## Patch fixes

* Units of variables in `calc_stratified_mean()` correctly represent data
* `get_clam_survey_data()` reworked to use updated strata definitions from 2018 and
resolved issue of dropped data
