...
lwfall <- lwpull$data |>
  dplyr::select(.data$SVSPP, .data$CATCHSEX, .data$SVLWCOEFF_FALL)
...
LWsexed <- combined_lw_species |>
  ...
  dplyr::mutate(SEX_DESCRIPTION = dplyr::case_when(
    CATCHSEX == 0 ~ "Unsexed",
    CATCHSEX == 1 ~ "Male",
    CATCHSEX == 2 ~ "Female",
...
### NOTE: NEW DATASET DOESN'T HAVE lna
...
### THE BELOW CODE WILL NEED TO BE REWRITTEN. MISSING lna and b USING SURVDAT LENGTHS. 
###########################################
### Calculate species condition ###
