...
apply_conversion_factors <- function(channel, survdat.raw, use.SAD = F) {
  ...
  for (i in 1:length(gcf.spp)) {
    survdat[
      SEASON == 'SPRING' & YEAR > 1972 & YEAR < 1982 & SVSPP == gcf.spp[i],
  ...
  big.abund <- data.table::data.table(
    svspp = c(
      '012',
      '022',
      '024',
  ...
  for (ispp in 1:length(spp.both)) {
    survdat[
      SVVESSEL %in% c('HB', 'PC') & SVSPP == spp.both[ispp],
  ...
  for (ispp in 1:length(spp.seasonal)) {
    survdat[
      SVVESSEL %in%
        c('HB', 'PC') &
        SEASON == 'SPRING' &
        SVSPP == spp.seasonal[ispp],
      BIOMASS := BIOMASS /
        big.bio[svspp == spp.seasonal[ispp] & season == 'spring', rhoW]
    ]
    ...
    survdat[
      SVVESSEL %in%
        ...
        SVSPP == spp.seasonal[ispp],
      BIOMASS := BIOMASS /
        big.bio[svspp == spp.seasonal[ispp] & season == 'fall', rhoW]
    ]
